...
import { Mail, Zap, CheckCircle2, Clock, FileEdit, XCircle, RefreshCw, TrendingUp, Activity, LayoutDashboard, ChevronDown, ChevronUp, Send, AlertCircle, BarChart2, MousePointer, Eye, UserMinus, DollarSign } from 'lucide-react';
...
interface CampaignMetric { campaign_id?: string; statistics?: { open_rate?: number; click_rate?: number; unsubscribe_rate?: number; bounce_rate?: number; delivered?: number; open_unique?: number; click_unique?: number; unsubscribe?: number; bounce?: number; revenue?: number; }; }
...
try {
const [cRes, fRes, mRes] = await Promise.all([fetch('/api/campaigns'), fetch('/api/flows'), fetch('/api/metrics')]);
const [cData, fData, mData] = await Promise.all([cRes.json(), fRes.json(), mRes.json()]);
...
// Build enriched metrics rows (join with campaign names)
const campaignNameMap: Record<string, string> = {};
campaigns.forEach(c => { campaignNameMap[c.id] = c.attributes.name; });
const sortedMetrics = [...metrics]
.filter(m => m.campaign_id && m.statistics)
.sort((a, b) => {
const av = a.statistics?.[metricsSort] ?? 0;
const bv = b.statistics?.[metricsSort] ?? 0;
return (bv as number) - (av as number);
});

// Aggregate totals from metrics if overview is empty
...
{activeTab === 'metrics' && <>
{metricsError ? (
<div style={{ background: '#1a0a0a', border: '1px solid #f8717133', borderRadius: 14, padding: 32, textAlign: 'center' }}>
<AlertCircle size={32} color="#f87171" style={{ margin: '0 auto 12px' }} />
<div style={{ color: '#f87171', fontWeight: 600, marginBottom: 8 }}>Metrics unavailable</div>
<div style={{ color: '#6b7280', fontSize: 12, maxWidth: 500, margin: '0 auto' }}>The Klaviyo Campaign Values Report API returned an error. Your API key may not have the Analytics permission enabled. Please check your API key scopes in Klaviyo.</div>
</div>
) : metrics.length === 0 ? (
<div style={{ background: '#0f0f12', border: '1px solid #1f1f2e', borderRadius: 14, padding: 48, textAlign: 'center', color: '#4b5563', fontSize: 13 }}>No metrics data available for the last 90 days.</div>
) : (<>
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 24 }}>
<MetricStatCard label="Avg Open Rate (90d)" value={fmtPct(avgOpenRate)} icon={<Eye size={20} />} color="#6366f1" sub={`${fmtNum(aggOpens)} unique opens`} />
<MetricStatCard label="Avg Click Rate (90d)" value={fmtPct(avgClickRate)} icon={<MousePointer size={20} />} color="#22c55e" sub={`${fmtNum(aggClicks)} unique clicks`} />
<MetricStatCard label="Total Delivered (90d)" value={fmtNum(aggDelivered || metricsOverview.total_delivered)} icon={<Send size={20} />} color="#38bdf8" sub={`${metrics.length} campaigns tracked`} />
<MetricStatCard label="Total Revenue (90d)" value={fmtAUD(aggRevenue || metricsOverview.total_revenue)} icon={<DollarSign size={20} />} color="#f97316" sub="Attributed revenue" />
</div>
<div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center' }}>
<span style={{ fontSize: 12, color: '#6b7280', fontWeight: 600, marginRight: 4 }}>Sort by:</span>
{([['open_rate', 'Open Rate'], ['click_rate', 'Click Rate'], ['delivered', 'Delivered'], ['revenue', 'Revenue']] as const).map(([key, label]) => (
<button key={key} onClick={() => setMetricsSort(key)} style={{ padding: '5px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: metricsSort === key ? '#1f1f2e' : 'transparent', color: metricsSort === key ? '#fff' : '#6b7280', border: `1px solid ${metricsSort === key ? '#6366f1' : '#1f1f2e'}`, cursor: 'pointer' }}>{label}</button>
))}
</div>
<div style={{ background: '#0f0f12', border: '1px solid #1f1f2e', borderRadius: 14, overflow: 'hidden' }}>
<table style={{ width: '100%', borderCollapse: 'collapse' }}>
<thead><tr style={{ borderBottom: '1px solid #1f1f2e' }}>
{['Campaign Name', 'Delivered', 'Open Rate', 'Click Rate', 'Unsub Rate', 'Revenue'].map(h => <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#4b5563', textTransform: 'uppercase' }}>{h}</th>)}
</tr></thead>
<tbody>
{sortedMetrics.map((m, i) => {
const name = m.campaign_id ? (campaignNameMap[m.campaign_id] || m.campaign_id) : 'Unknown';
const s = m.statistics || {};
return (
<tr key={m.campaign_id || i} className="rhover" style={{ borderBottom: '1px solid #1a1a26', background: i % 2 === 0 ? '#0f0f12' : '#0b0b10' }}>
<td style={{ padding: '12px 16px' }}><div style={{ fontSize: 12, fontWeight: 600, color: '#e5e7eb', maxWidth: 300 }}>{name}</div></td>
<td style={{ padding: '12px 16px', fontSize: 12, color: '#9ca3af' }}>{fmtNum(s.delivered)}</td>
<td style={{ padding: '12px 16px', minWidth: 140 }}><RateBar value={s.open_rate ?? 0} color="#6366f1" /></td>
<td style={{ padding: '12px 16px', minWidth: 140 }}><RateBar value={s.click_rate ?? 0} color="#22c55e" /></td>
<td style={{ padding: '12px 16px', minWidth: 140 }}><RateBar value={s.unsubscribe_rate ?? 0} color="#f87171" /></td>
<td style={{ padding: '12px 16px', fontSize: 12, color: '#f97316', fontWeight: 600 }}>{s.revenue != null ? fmtAUD(s.revenue) : '-'}</td>
</tr>
);
})}
{sortedMetrics.length === 0 && <tr><td colSpan={6} style={{ padding: 48, textAlign: 'center', color: '#4b5563', fontSize: 13 }}>No campaign metrics found</td></tr>}
</tbody>
</table>
</div>
</>)}

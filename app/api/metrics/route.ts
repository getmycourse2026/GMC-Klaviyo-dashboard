...
export async function GET() {
  ...
  const headers = { Authorization: `Klaviyo-API-Key ${apiKey}`, revision: '2024-10-15', Accept: 'application/json', 'Content-Type': 'application/json' };
  let metricId = '';
  try {
    const mr = await fetch('https://a.klaviyo.com/api/metrics/?filter=equals(name,"Placed Order")&page[size]=1', { headers: { Authorization: `Klaviyo-API-Key ${apiKey}`, revision: '2024-10-15', Accept: 'application/json' }, cache: 'no-store' });
    if (mr.ok) { const md = await mr.json() as { data?: Array<{ id: string }> }; metricId = md.data?.[0]?.id || ''; }
  } catch (_) {}
  if (!metricId) {
    try {
      const mr2 = await fetch('https://a.klaviyo.com/api/metrics/?page[size]=1', { headers: { Authorization: `Klaviyo-API-Key ${apiKey}`, revision: '2024-10-15', Accept: 'application/json' }, cache: 'no-store' });
      if (mr2.ok) { const md2 = await mr2.json() as { data?: Array<{ id: string }> }; metricId = md2.data?.[0]?.id || ''; }
    } catch (_) {}
  }
  ...
  const res = await fetch('https://a.klaviyo.com/api/campaign-values-reports/', { method: 'POST', headers, body, cache: 'no-store' });
